...
THREE.Pillar = function (position, size, color, length, flow) {
	...
	sparksEmitter.addInitializer(new SPARKS.Velocity(new SPARKS.PointZone(new THREE.Vector3(0, -5, 0)))); //设置速度
	...
	function generateSprite(color) {
		...
		gradient.addColorStop(0.1, 'rgba(' + Math.round(color.r * 255) + ',' + Math.round(color.g * 255) + ',' + Math.round(color.b * 255) + ',0.8)');
		gradient.addColorStop(0.6, 'rgba(' + Math.round(color.r * 255 / 2 - 100) + ',' + Math.round(color.g * 255 / 1.5 - 100) + ',' + Math.round(color.b * 255 / 1.5 - 100) + ',0.5)');
